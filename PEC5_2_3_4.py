...
linesDF = spark\
    ...
    .option('port', 20036)\
    .option('sql.files.minPartitionNum',4) \
    .load()
...
vuelosDF = vuelosDF.dropDuplicates(['callsign'])

vuelosDF = vuelosDF.groupBy("country").count()

vuelosDF = vuelosDF.orderBy("country")
...
while query.isActive:
    ...
    display(spark.sql('SELECT * FROM vuelos').show())
    sleep(5)
